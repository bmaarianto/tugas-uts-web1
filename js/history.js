...
function checkUser() {
  ...
  if (currentUser.role !== "Admin") {
    document.querySelector(".card-green").style.display = "none"; // Total Pendapatan
    document.querySelector(".card-orange").style.display = "none"; // Total Buku Terjual

    // Adjust grid layout for remaining card
  ...
  if (savedTransactions) {
    ...
    allTransactions = [
      ...
      },
      {
        ...
        items: [{ nama: "Kepemimpinan", qty: 3, harga: 150000 }],
        subtotal: 450000,
        ongkir: 12000,
        total: 462000,
        status: "dikirim",
      ...
      },
      {
        ...
        items: [{ nama: "Pengantar Ilmu Komunikasi", qty: 1, harga: 180000 }],
        subtotal: 180000,
        ongkir: 13000,
        total: 193000,
        status: "pending",
...
function renderTransactions() {
  ...
  filteredTransactions.forEach((transaction, index) => {
    ...
    row.innerHTML = `
                    ...
                    <td><strong>Rp ${transaction.total.toLocaleString(
                      "id-ID"
                    )}</strong></td>
                    <td><span class="badge ${
...
function showDetail(index) {
  ...
  let itemsHtml =
    '<div class="items-table"><h3>Daftar Item</h3><table style="width: 100%; margin-top: 10px;"><thead><tr><th>Nama Buku</th><th>Qty</th><th>Harga</th><th>Subtotal</th></tr></thead><tbody>';
  ...
  transaction.items.forEach((item) => {
    itemsHtml += `
                    <tr>
                        ...
                        <td><strong>Rp ${(item.qty * item.harga).toLocaleString(
                          "id-ID"
                        )}</strong></td>
                    </tr>
                `;
  ...
  document.getElementById("detailContent").innerHTML = `
                ...
                <div class="detail-row" style="margin-top: 20px;">
                    ...
                    <span class="detail-value">Rp ${transaction.subtotal.toLocaleString(
                      "id-ID"
                    )}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Ongkos Kirim</span>
                    <span class="detail-value">Rp ${transaction.ongkir.toLocaleString(
                      "id-ID"
                    )}</span>
                </div>
                <div class="detail-row" style="background: #f8f9fa; padding: 15px; border-radius: 5px; font-size: 18px;">
                    ...
                    <span class="detail-value"><strong style="color: #667eea;">Rp ${transaction.total.toLocaleString(
                      "id-ID"
                    )}</strong></span>
                </div>
            `;
